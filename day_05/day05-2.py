...
for _instructions in move_instructions:
    ...
    for _i in range(count_moved):
        _move_list.append(stack_list[from_where].pop())
        print(f"Intermediate: {stack_list}")

    print(f"Move List: {_move_list} with target {stack_list[to_where]}")

    _move_list.reverse()

    for _x in _move_list:
        stack_list[to_where].append(_x)

    print(f"After adding move list: {stack_list[to_where]}")
